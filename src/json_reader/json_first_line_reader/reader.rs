...
mod tests {
    ...
    fn test_json_first_line() {
        ...
        assert_eq!("processId", item.name.as_str(&parser).unwrap());
        assert_eq!(
            "8269e2ac-fa3b-419a-8e65-1a606ba07942",
            item.value.as_str(&parser).unwrap()
        );

        let item = parser.get_next().unwrap().unwrap();

        assert_eq!("sellAmount", item.name.as_str(&parser).unwrap());
        assert_eq!("0.4", item.value.as_str(&parser).unwrap());

        let item = parser.get_next().unwrap().unwrap();

        assert_eq!("buyAmount", item.name.as_str(&parser).unwrap());
    ...
    pub fn test_with_array_inside_json_l_split() {
        ...
        while let Some(sub_json) = first_line_reader.get_next() {
            ...
            println!(
                "{}",
                sub_json.name.as_str(&first_line_reader).unwrap(),
            );
        }
    }
